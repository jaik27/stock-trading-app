...
class socket(Emitter.emitter):
        ...
        if message == "":
            ...
            if result == 1:
                ...
                if self.OnAuthentication is not None:
                    ...
                    sclogger.debug("authentication result received: " + str(dataobject))
    ...
    def on_open(self, ws):
        print(f"Socketcluster: on_open called, authToken being sent: {self.authToken}")
        self.resetvalue()
        handshakeobject = json.loads('{}')
        handshakeobject["event"] = "#handshake"
        handshakeobject["data"] = {}
    ...
    def setAuthtoken(self, token):
        print(f"Socketcluster: setAuthtoken called with token: {token}")
