...
class DataPreparation:
    ...
    def prepare_scalping_data(self):
        ...
        try:
            ...
            self.db_cursor.execute("SELECT * FROM get_latest_scalping_data(10)")
    ...
    def create_feature_engineering_functions(self):
        ...
        try:
            # --- OFFLINE/HISTORICAL (1min) MODE: Use this logic ---
            self.db_cursor.execute("""
                ...
                $$ LANGUAGE plpgsql;
            """)
            # PATCH: Add other feature engineering functions here as needed
            self.db_conn.commit()
    ...
    def prepare_ml_training_data(self):
        ...
        try:
            # Try NIFTY/BANKNIFTY with data, else fallback to any instrument with data
            ...
            # Log available technical_indicators and candle_data_1min for the chosen instrument
            ...
            # Build dataframe and continue as before
            columns = [
    ...
    def prepare_llm_data(self):
        try:
            # LLM data logic unchanged, operates on all symbols in 1-min view
            self.db_cursor.execute("""
                ...
                WITH latest_prices AS (
                    ...
                    WHERE cd."time" >= NOW() - INTERVAL '30 minutes'
                ...
                latest_indicators AS (
                    ...
                    WHERE 
                        ti.timeframe = '1min' AND
                        ti."time" >= NOW() - INTERVAL '30 minutes'
    ...
    def create_scalping_signal_generator(self):
        # This method is unchanged; it can be updated later to use 1min or 15sec as needed.
        logger.info("Scalping signal generator script created successfully")
        return True
